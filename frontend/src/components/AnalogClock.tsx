...
type ClockHandRotations = {
  hour: number;
  minute: number;
  second: number;
};

function handRotation(date: Date): ClockHandRotations {
...
function ClockHands({ rotations }: { rotations: ClockHandRotations }) {
  return (
    <>
      <div
...
export function AnalogClock() {
  ...
  return (
    <div className="relative aspect-square w-full max-w-[min(82vw,28rem)] rounded-full border border-neutral-700 bg-neutral-900 shadow-2xl shadow-black/30">
      <ClockTickMarks />
      <ClockNumbers />
      <ClockHands rotations={rotations} />
    </div>
  );
}
